...
const paddockManagers = [
  ...
  */
  ...
  function listPaddockManagersByName() {
    return paddockManagers.map((paddockManager) => paddockManager.taxNumber).sort(paddockManagers.name);
  };
  ...
  function sortPaddockTypeByTotalArea() {
    const paddocksSum = paddocks.reduce((acumulador, valorActual) => {
        const elementYaExiste = acumulador.find(element => element.farmId === valorActual.farmId);
        
        if (elementYaExiste) {
          return acumulador.map((element) => {

            if (element.farmId === valorActual.farmId) {    
              return {
                ...element,
                area: element.area + valorActual.area
              }
            }  
            return element;
          });
      ...
      let  [,,farmId,,area] = paddocksSum
      return (farmId + area) 

      
    // const arrayfarms = farms.map((farms) => farms)
    // const arraypaddocks = paddocks.map((paddock) => paddocks)
    // for(var i = 0; i<arrayfarms.length;i++){
    //     for(var a = 0; a<arraypaddocks.length;a++){
    //         let id = arrayfarms.find(x => x === )
    //     }
    // }
    // return arrayfarms[2];
  }
  ...
  function sortFarmManagerByAdminArea() {
    ...
    let bankEstruct = paddocks.reduce((contadorTotales, cadaElemento) => {
        contadorTotales[cadaElemento.paddockManagerId] = (contadorTotales[cadaElemento.paddockManagerId] || 0);
